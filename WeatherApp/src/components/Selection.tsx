...
const Selection = ({ targetPlace }: { targetPlace: string }) => {
  ...
  return (
    <div className={"h-full bg-gray-100 flex flex-col justify-around"}>
      {weather && (
        <div
          className={"flex justify-around mt-5 pb-5 border-b border-gray-400"}
        >
          <div>
            <h2 className={"text-xl font-bold"}>{day().split(",")[0]}</h2>
            <h4 className={"text-lg font-semibold"}>
              {weather[0]?.city_name},{" "}
              {weather[0]?.country_code == "IN"
                ? "India"
                : weather[0]?.country_code}
            </h4>
          </div>
          <div>
            ...
            <div className={"flex justify-between text-center"}>
              <div>
                ...
                <h4 className={"text-lg"}>
                  {utcTimeNow(weather[0]?.sunrise)} am
                </h4>
              </div>
              <div>
                <div className={"font-bold text-lg"}>Sunset</div>
                <div className={"text-lg"}>
                  {utcTimeNow(weather[0]?.sunset)} pm
                </div>
              </div>
            </div>
          </div>
              ...
              <div className={"flex gap-5 text-center items-center"}>
                <div>
                  <h3 className={"text-2xl font-bold"}>
                    {Number(weather[0]?.uv).toFixed(2)} UVI
                  </h3>
                </div>
                <div
                  className={`${
                    uvIndex(weather[0]?.uv) == ("Low" || "moderate")
                      ? "bg-lime-300 text-black"
                      : "bg-red-600 text-white"
                  } text-center p-3 pt-1 pb-1 rounded-xl`}
                >
                  <h4 className={"text-sm font-semibold"}>
                    {uvIndex(weather[0]?.uv)}
                  </h4>
                </div>
              </div>
              <div>
                <h5 className={"text-lg text-gray-300"}>
                  {uvIndex(weather[0]?.uv)} risk of from UV rays
                </h5>
              </div>
            </div>
          </div>
            ...
            <div className={"flex flex-col gap-3"}>
                ...
                <div className={"flex flex-col w-full justify-around"}>
                  <div>
                    <h3 className={"text-gray-600 font-medium text-sm"}>
                      {weather2[1].valid_date}
                    </h3>
                  </div>
                  <div className={"flex justify-between"}>
                    <div>
                      <h2 className={"font-bold text-lg"}>
                        {weather2[1].weather["description"].split(" ").length >
                        1
                    ...
                    <div className={"text-end"}>
                      <h2 className={"text-gray-700 text-xl font-bold"}>
                        {Math.round(Number(weather2[1]?.max_temp))}° /{" "}
                        {Math.round(Number(weather2[1]?.min_temp))}°
                      </h2>
                    </div>
                  </div>
                </div>
              ...
              >
                ...
                <div className={"flex flex-col w-full justify-around"}>
                  <div>
                    <h3 className={"text-gray-600 font-medium text-sm"}>
                      {weather2[2].valid_date}
                    </h3>
                  </div>
                  <div className={"flex justify-between"}>
                    <div>
                      <h2 className={"font-bold text-lg"}>
                        {weather2[2].weather["description"].split(" ").length >
                        1
                    ...
                    <div className={"text-end"}>
                      <h2 className={"text-gray-700 text-xl font-bold"}>
                        {Math.round(Number(weather2[2]?.max_temp))}° /{" "}
                        {Math.round(Number(weather2[2]?.min_temp))}°
                      </h2>
                    </div>
                  </div>
                </div>
